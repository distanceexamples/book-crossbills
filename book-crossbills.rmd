---
title: "Lure point transect surveys from Buckland et al. (2015)"
description: |
...
The trials data are in file `lure-trials.csv`.  Import the data and check that it has been read correctly.  Files needed for this exercise can be found on [this Github repository](https://github.com/distanceexamples/book-crossbills).
...
<aside>
```{r ref.label='boxes', echo=FALSE, fig.width=6, fig.height=6}
```
</aside>
...
<aside>
```{r  ref.label='detfn', echo=FALSE, fig.height=6, fig.width=6}
```
</aside>
...
<aside>
```{r ref.label='triang', echo=FALSE, fig.width=3.5, fig.height=3.5}
```
</aside>
...
<aside>
**Note:** The point estimate produced here differs from the estimates shown in [@SUMMERS_2010] because this simplified analysis has omitted corrections for unsuitable habitat on surveyed plots, for juveniles, for unidentified crossbills, for incubating or brooding females, and for flying birds.
</aside>
...
```{r boothist, echo=TRUE, eval=answer, fig.cap="Distribution of abundance estimates from bootstrap.", message=FALSE, tidy=FALSE}
alpha <- 0.05
...
<aside>
__Note:__  The distribution of estimates is skewed-right; long tail of the distribution is to the right.  This shape is customary in many abundance estimation problems.  Vertical lines are confidence interval bounds; values provided below
</aside>
